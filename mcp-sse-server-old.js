...
app.get('/mcp/sse', async (req, res) => {
    ...
    const server = new Server({ name: 'whatsapp-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
    // Handle tool calls
    server.setRequestHandler('tools/list', async () => ({
        tools: [
            { name: 'whatsapp_list_chats', description: 'List chats', inputSchema: { type: 'object', properties: {} } },
            { name: 'whatsapp_get_connection_status', description: 'Get status', inputSchema: { type: 'object', properties: {} } }
        ]
    }));
