...
const createLogger = (name: string) => ({
  info: (msg: unknown, meta?: unknown) => console.log(`[${name}]`, msg, meta || ''),
  error: (msg: unknown, meta?: unknown) => console.error(`[${name}]`, msg, meta || ''),
  warn: (msg: unknown, meta?: unknown) => console.warn(`[${name}]`, msg, meta || '')
});
...
// ==================== Database Helpers ====================
async function getUserByPhone(phone: string): Promise<WaUser | null> {
...
async function searchContacts(userId: string, query: string, limit: number = 20) {
  const { data, error } = await supabase
    .from('wa_contacts')
    .select('*')
    .eq('user_id', userId)
...
async function listChats(userId: string, limit: number = 50, offset: number = 0) {
  const { data, error } = await supabase
    .from('wa_chats')
    .select('*')
    .eq('user_id', userId)
...
async function getChatByJid(userId: string, jid: string) {
  const { data, error } = await supabase
    .from('wa_chats')
    .select('*')
    .eq('user_id', userId)
...
async function listMessages(userId: string, chatJid: string, limit: number = 50) {
  const { data, error } = await supabase
    .from('wa_messages')
    .select('*')
    .eq('user_id', userId)
...
async function searchMessages(userId: string, query: string, chatJid?: string, limit: number = 20) {
  let q = supabase
    .from('wa_messages')
    .select('*')
    .eq('user_id', userId)
...
// ==================== MCP Server Factory ====================
function createMcpServer(user: WaUser): Server {
  const server = new Server(
    { name: 'whatsapp-mcp-sse', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
    ...
    try {
      switch (name) {
        ...
        case 'whatsapp_get_chat': {
          const { chat_jid } = args as { chat_jid: string };
        ...
        case 'whatsapp_list_messages': {
          const { chat_jid, limit = 50 } = args as { chat_jid: string; limit?: number };
        ...
        case 'whatsapp_search_messages': {
          const { query, chat_jid, limit = 20 } = args as { query: string; chat_jid?: string; limit?: number };
